...
import { deleteMatchRecord } from '@/firebase/services/deleteServices';

interface matchedUserTable {
  data: any;
}

const MatchedUsersTable: React.FC<matchedUserTable> = ({ data }) => {
  //states
  ...
  const handleDelete = async (row: any) => {
    setLoading(true);
    const response = await deleteMatchRecord(row?.original?.userInfo?.uid, row?.original?.otherInfo?.uid);
    setLoading(false);
    return response;
  };
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      header: 'Status',
      accessorKey: 'action',
      cell: () => <span className={`text-xs font-medium me-2 px-2.5 py-0.5 rounded-full text-white bg-success`}>Matched</span>,
    },
    {
      header: 'Action',
      cell: ({ row }) => (
        <div className="flex gap-x-2 ">
          <DeleteModal
            ...
            row={row}
            title="Are you sure you want to delete this match card and also delete related data like conversation, etc. If exit?"
            loading={loading}
          />
        </div>
      ),
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white text-black dark:text-white px-5 pt-4 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <TableTitle globalFilter={globalFilter} setGlobalFilter={setGlobalFilter} tableName="Matched Users" />

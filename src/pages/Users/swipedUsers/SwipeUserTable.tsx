...
import { deleteSwipeCard } from '@/firebase/services/deleteServices';
...
const SwipeUserTable: React.FC<swipedUsersTale> = ({ data }) => {
  ...
  const handleDelete = async (row: any) => {
    setLoading(true);
    const response = await deleteSwipeCard(row?.original?.userInfo?.uid, row?.original?.otherInfo?.uid);
    setLoading(false);
    return response;
  };
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <span className={`text-xs font-medium me-2 px-2.5 py-0.5 rounded-full `}>{moment.unix(row.original.createdAt).format('DD MMM, YYYY hh:mm a')}</span>,
    },
    {
      header: ({ column }) => (
        <Button className="px-0 " variant="ghost" onClick={() => column.toggleSorting(column.getIsSorted() === 'asc')}>
      ...
      accessorKey: 'action',
      cell: ({ row }) => <span className={`text-xs font-medium me-2 px-2.5 py-0.5 rounded-full text-white ${getBadge(row.original.action)}`}>{row.original.action}</span>,
    },
    {
      header: 'Action',
      cell: ({ row }) => (
        <div className="flex gap-x-2 ">
          <DeleteModal
            ...
            row={row}
            title="Are you sure you want to delete this swiped card and also delete related data like matches, conversation, etc. If exit?"
            loading={loading}
          />
        </div>
      ),
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white text-black dark:text-white px-5 pt-4 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <TableTitle globalFilter={globalFilter} setGlobalFilter={setGlobalFilter} tableName="Swiped Users" />
